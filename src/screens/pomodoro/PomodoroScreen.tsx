...
const AMBIENT_OPTIONS: Array<{ label: string; value: 'none' | 'rain' | 'lofi' | 'whitenoise' }> = [
...
export function PomodoroScreen() {
  ...
  const { focusMinutes, breakMinutes, longBreakMinutes, sessionsUntilLongBreak, setDurations } = usePomodoroStore();
  const { phase, secondsLeft, isRunning, completedBlocks, progress, start, pause, reset, skip } = usePomodoro(taskId);
  ...
  return (
    <ScreenContainer>
      ...
      {linkedTask ? (
        <Text style={[styles.linkedTask, { color: theme.textSecondary }]}>Working on: {linkedTask.title}</Text>
      ) : (
      ...
      <View style={styles.durationRow}>
        {[15, 25, 45, 60].map((min) => (
          <Pressable
            ...
            style={[
              styles.durationChip,
              { backgroundColor: focusMinutes === min ? theme.primary : theme.surface, borderColor: theme.border },
            ]}
          >
      ...
      <View style={styles.durationRow}>
        {AMBIENT_OPTIONS.map((opt) => (
          <View key={opt.value} style={[styles.durationChip, { borderColor: theme.border, backgroundColor: theme.surface }]}>
            <Text style={{ color: theme.textSecondary, fontSize: typography.size.sm }}>{opt.label}</Text>
          </View>
        ))}
      </View>
      <Text style={[styles.note, { color: theme.textMuted }]}>
        Ambient audio needs a bundled sound file (expo-av) — drop your own .mp3s into assets/sounds and wire them
        up here; left as a hook point to keep this starter lightweight.
      </Text>
    </ScreenContainer>
  );
}
...
const styles = StyleSheet.create({
  heading: { fontFamily: typography.fontFamily.headingBold, fontSize: typography.size.xxl, textAlign: 'center' },
  linkedTask: { fontFamily: typography.fontFamily.body, fontSize: typography.size.sm, textAlign: 'center', marginTop: 4, marginBottom: spacing.lg },
  ...
  blocksText: { textAlign: 'center', fontFamily: typography.fontFamily.body, fontSize: typography.size.sm, marginBottom: spacing.lg },
  controlsRow: { flexDirection: 'row', justifyContent: 'center', gap: spacing.sm, marginBottom: spacing.xl },
  sectionTitle: { fontFamily: typography.fontFamily.heading, fontSize: typography.size.md, marginBottom: spacing.sm },
  durationRow: { flexDirection: 'row', flexWrap: 'wrap', gap: spacing.xs, marginBottom: spacing.md },
  durationChip: { paddingHorizontal: spacing.md, paddingVertical: 8, borderRadius: radius.pill, borderWidth: 1 },
  note: { fontFamily: typography.fontFamily.body, fontSize: typography.size.xs, lineHeight: 16, marginTop: spacing.xs },
});
