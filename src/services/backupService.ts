...
export async function exportBackup(): Promise<void> {
  ...
  if (await Sharing.isAvailableAsync()) {
    await Sharing.shareAsync(fileUri, {
      ...
      dialogTitle: 'Export BloomDaily data',
    });
  }
}
...
export async function importBackup(): Promise<{ success: boolean; message: string }> {
  ...
  try {
    ...
    // Backups made before cloud sync was added won't have `updatedAt` on
    // each task/habit — backfill it so sync merging has something sane to
    // compare against instead of `undefined`.
    const normalizedTasks = (payload.tasks ?? []).map((t) => ({
      ...t,
      updatedAt: t.updatedAt ?? t.createdAt ?? new Date().toISOString(),
    }));
    const normalizedHabits = (payload.wellness.habits ?? []).map((h) => ({
      ...h,
